...
Shadows have to implement a field where they store their communication state. As we cannot connect to a tag that is bound in communication, we have to wait for it to leave the communication to be able to perform roaming actions.
```mermaid
sequenceDiagram
    autonumber
    participant Gateway
